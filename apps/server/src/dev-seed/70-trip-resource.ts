 ...
 * 交通数据都没有** —— 那三种卡加上页尾的「未定发车时间」块，谁都没在页面上
 * 见过。管理端的行程页和资源页同样是空的。
 *
 ...
 * | 王芳的高铁（活动后一天） | 票务卡 + 「返程日」 |
 * | 王芳的接驳车（有发车时间） | 用车卡，混排进当天时间轴，司机电话可拨 |
 * | 王芳的送站车（**没有**发车时间） | 页尾「用车安排（未定发车时间）」那一块 |
...
export const seed: SeedFn = async (db, { userId }) => {
  ...
  await db.insert(activityResource).values([
    ...
    },
    {
      ...
      name: "闭幕送站专车",
      quantity: 1,
      // **刻意不填时间**：这一列可空，而 h5 不会把没有发车时间的用车塞进某一天
      // 去猜，它单列在页尾。不留一条这样的数据，那一块永远不出现。
      startTime: null,
      endTime: null,
      location: "A 馆东门 贵宾通道",

...
> 本节是**给做角色管理那一轮的交接**，不是已经拍板的设计。落地时整节替换成真实设计。
> 上面 §1–§9 全部已实现并合入 master，那些是**已定的约束**，不要推翻重来。

### 10.1 已经建好、不用再做的

| 东西 | 位置 | 状态 |
| --- | --- | --- |
| `role` 表 | `modules/user/schema.ts` | 已建。`id / name(唯一) / remark / createdBy / updatedBy / 时间戳`，**刻意没有 status、没有 roleKey、没有排序字段**（理由见 §5.4） |
| `user_role` 表 | 同上 | 已建。复合主键，多对多。`userId` cascade、`roleId` NO ACTION（还有人挂着的角色不能删） |
| `/api/role/list` | `modules/user/routes.role.ts` | 已通。不分页，返回 `{id,name,remark}` |
| 用户挂角色 | `modules/user/routes.ts` 的 `create`/`update` | **已经收 `roleIds` 并写库**，列表和详情也已返回 `roles`。角色管理这一步不用回头改用户模块 |
| 角色管理菜单入口 | `app/nav.ts` | 已有，指向 `/system/role` |
| `/system/role` 页面 | `routes/_authenticated/system/role.tsx` | **还是 `PagePlaceholder`**，要替换 |
| 内置超管角色 | `bootstrap.ts` 的 `BUILTIN_ROLE_NAME`（"超级管理员"） | 生产引导和 dev-seed 都会建它 |

### 10.2 当前的中间态（重要）

**现在任何登录用户都能调任何接口。** 全站 137 条业务接口只挂了 `requireUser`，没有任何按角色
或权限点的判断——角色目前纯粹是"能选能存能显示"的标签。这是 §1 决策 5 说好的中间态，闸门随
角色管理一起上。

所以角色管理这一轮的真正工作量**不在角色的 CRUD**（那是照抄 supplier 的一天活），而在**把闸门
装到 137 条接口和一整棵菜单上**。

### 10.3 已定的约束（不要推翻）

- **权限点在代码里，角色在数据库里**（§3）。运营能自由建角色、自由组合已有权限点，但发明不出
  新权限点。
- **不做菜单表入库**。`apps/web` 是文件路由，库里加一行菜单指向不存在的路由只会得到 404 ——
  菜单表的"增删改"对我们是假能力，只有"隐藏一个已存在的页面"是真的。
- **权限取并集，没有"拒绝"语义**（§5.4）。
- **不做数据权限 `dataScope`**（§2）。
- **前端菜单过滤和路由守卫都不是安全边界**（AGENTS.md「渲染模型」）。每条受保护接口必须在
  handler 内部独立校验，前端过滤只是别让用户看见点不动的东西。

### 10.4 要 grill 的开放问题

按依赖顺序排，前面的不定后面的问不了。**每条都附了倾向，但都没拍板。**

**Q1 权限点的粒度。** 137 条接口逐条列 = 137 个复选框，没人配得动；只按页面列（约 20 个）又
表达不了"能看不能改"。倾向：**按「页面 × 动作」两维**，页面约 20 个，动作收敛成
`view / edit / delete` 三档，落成 `supplier:view` 这种 key，约 60 个点，按页面分组勾选。
待确认的是三档够不够——比如"导出"和"导入"要不要单列。

**Q2 一条接口归哪个权限点。** 有了 key 还要有映射。倾向：**在路由链上显式挂**
（`.use(requirePermission("supplier:edit"))` 或逐条），而不是靠路径猜。`member/routes.relation.ts`
一个文件 19 条接口横跨三层，猜不出来。

**Q3 超管怎么绕过。** 倾向：**不给超管特判**，而是让"超级管理员"角色**拥有全部权限点**（建角色
时把清单灌满）。特判 `isBuiltin` 会让"超管为什么能点这个"变成读代码才知道的事。但代价是新增
权限点时要记得补进这个角色——需要一个测试盯住。

**Q4 前端怎么拿到权限。** session 里带回当前用户的权限点集合？还是单独一个 `/api/auth/permissions`？
倾向：**跟 session 一起回**，少一次往返，且天然跟登录态同生命周期。

**Q5 菜单和按钮怎么过滤。** `nav.ts` 现在是纯静态数组。倾向：给 `NavLeaf` 加一个可选的
`permission` 字段，渲染时过滤；按钮级用一个 `<Can permission="...">` 或 `useCan()`。

**Q6 角色删除的保护。** `user_role.roleId` 是 NO ACTION，所以还有人挂着就删不掉——但那会抛
数据库错误。要在 handler 里先查一下给中文提示。另外**内置的"超级管理员"角色不能删**，
同 `isBuiltin` 的思路。

**Q7 会不会把自己锁死。** 管理员改自己所属角色的权限，可能一提交就没了进后台的权限。倾向：
禁止编辑"超级管理员"角色的权限点（它恒等于全集），其余角色允许——因为总还有超管能救。

### 10.5 动手前必读

- 本文档 §3（为什么不做菜单表）、§5.4（角色表为什么长这样）、§10.3。
- [crud-page-guide.md](crud-page-guide.md) —— 角色管理页的表格/弹窗/筛选栏形态照它抄。
- `modules/supplier/` + `routes/_authenticated/supplier/` —— 新代码的范式。
- `modules/user/routes.ts` —— 同一模块里已经写好的那套，角色的 CRUD 直接对齐它。

...
- 用车统一归入「我的行程」；
- 已关联到当前嘉宾可见环节的用车展示在对应环节内，作为该环节的服务信息，不再
  单独计作一项；车辆自己的时间只表示实际发车时间，不拿环节时间冒充；
- 未关联到可见环节且缺发车时间的用车放「待定安排」，**计入总项数、不算天数**；
- 车程用结束减开始时间（分钟）算，**任一缺失则整个不显示**——禁止占位值。
- 跨天议程按 `Asia/Shanghai` 的自然日拆开展示：首日显示到 `24:00`，后续日期
  从 `00:00` 开始；接口仍保留完整时间段，顶部总项数按原始环节计一次。
- 环节可配置「在 H5 个人行程隐藏结束时间」；勾选后只隐藏行程时间栏里的结束
  时间，分天、排序和进行状态仍按数据库中的真实结束时间计算。
...
议程环节地点和已绑定人员的用车集合点都可带可选的百度 BD-09 定位点。H5 只在接口
返回定位点时显示「导航」按钮；地点只有文字时继续静态展示，不猜坐标。导航面板提供
百度地图、高德地图，以及支持时显示的苹果地图，并保留浏览器未安装地图 App 时的网页
路线兜底。具体坐标字段和坐标系转换见 [百度地图选点](baidu-location-picker.md)。

## 开发环境怎么进这一页

运行 `bun run dev`，使用启动输出的实际 H5 origin，访问 `/a/demo-itinerary`；种子手机号 `13810000000`（王芳）可验个人多座，`13810000001`（李强）可验没有个人排座时的团体座位。不要写死 3101 或猜管理端端口。
...
议程行上有个人座位号时多一颗「座位图」按钮，推上来一张**定位图**：这片区的座位
分布 + 我在哪。成员所属团体有占位时，即使本人已有固定座位，个人座位下方仍显示
“您的团体成员座位安排在 …”及范围；没有个人排座时，团体范围作为主位置并提供「团体
座位」入口。面板顶部突出显示范围、场地和议程，图中统一标红团体占用的位置，不暗示
其中有某个固定的个人座位。它自己一套设计取舍——坐标为什么在服务端解析、盒子高度怎么适应
从 3.2:1 到 ∞ 的长宽比、一万座为什么只有一个 DOM 节点、为什么不画舞台方位——见
[h5-seat-map.md](h5-seat-map.md)。**改那张图之前先读它**，尤其是"图上所有座位
长得一模一样"这条：一条 path 装下全部座位的前提就是它。

排位开关关闭后，行程不再返回该环节的座位展示数据：即使数据库里还留有历史已确认
方案，也不显示区域、座位号或「座位图」入口；直接请求座位图接口同样按没有本人座位
处理。这样管理端关闭排位后，嘉宾端不会继续看到旧座位信息。
